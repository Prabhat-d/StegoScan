...
async function runEmbed() {
  ...
  const embeddingProfile = document.getElementById("embedding-profile").value;
  ...
  if (hideType === "text") form.append("message", message);
  if (hideType === "file") form.append("hidden_file", hiddenFile);
  if (hideType === "image") form.append("hidden_image", hiddenImage);
  ...
  try {
    ...
    const stegoUrl = "data:image/png;base64," + data.stego;

    document.getElementById("embed-orig").src = coverUrl;
    document.getElementById("embed-stego").src = stegoUrl;

    const sliderOrig = document.getElementById("stego-slider-orig");
    const sliderStego = document.getElementById("stego-slider-stego");
    if (sliderOrig && sliderStego) {
      sliderOrig.src = coverUrl;
      sliderStego.src = stegoUrl;
    }

    const pct = ((data.bits_used / data.capacity) * 100).toFixed(2);
    document.getElementById("cap-label").textContent =
      `Capacity used: ${pct}% (${data.bits_used.toLocaleString()} of ${data.capacity.toLocaleString()} bits)`;
    document.getElementById("cap-fill").style.width = Math.max(pct, 0.5) + "%";

    document.getElementById("meta-bits").textContent = data.bits_used.toLocaleString();
    document.getElementById("meta-cap").textContent = data.capacity.toLocaleString();
